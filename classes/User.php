...
/*
    ...
    require       = same with include but will display error stop the script
    require_once  = same with include_once but will display error stop the script
*/
...
class User extends Database
{
    ...
    public function login($request)
    {
        ...
        if($result->num_rows == 1){
            $user = $result->fetch_assoc();
                ...
                /* $user = ['id' => 1, 'first_name' => 'Ayane', 'last_name' => 'Goto', 'usrname' => 'ayane',
                 'password' => '$2y$10$powKeRU', 'photo' => NULL];
                */ 

                /*
                    $user['id'] = get the value 1
                    $user['first_name'] = get the value 'Ayane'
                    $user['nast_name'] = get the value 'Goto'
                    $user['username'] = get the value 'ayane'
                    $user['password'] = get the value '$2y$10$powKeRU'
                */

            #check the password if correct
            if(password_verify($password, $user['password'])){
                # Creat session variables for future use
                session_start();
                $_SESSION['id']        = $user['id'];
    ...
    public function getAllUsers()
    {
        ...
        if($result = $this->conn->query($sql)){ // we use $result only if we use SELECT in $sql syntax
            return $result;
        }else{
